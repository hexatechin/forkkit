...
import { LayoutDashboard, Package, ClipboardList, Settings, LogOut, Store, FolderTree, Menu, X } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      ...
      <header className="lg:hidden sticky top-0 z-30 bg-white border-b flex items-center justify-between px-4 h-14">
        <button onClick={() => setSidebarOpen(true)} className="h-9 w-9 flex items-center justify-center rounded-lg hover:bg-neutral-100" aria-label="Open menu">
          <Menu className="h-5 w-5" />
        </button>
        <div className="flex items-center gap-2 font-bold">
          <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-orange-500 via-rose-500 to-fuchsia-500 text-white text-sm">K</span>
          Kirano
        </div>
        <div className="w-9" />
      </header>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 bg-black/40 backdrop-blur-sm z-40 animate-in fade-in" onClick={() => setSidebarOpen(false)} />
      )}
      ...
      >
        <div className="flex items-center justify-between font-bold px-4 py-4 lg:py-5">
          ...
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden h-8 w-8 flex items-center justify-center rounded-lg hover:bg-neutral-100">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="border-t p-4 shrink-0">
          <div className="px-1 py-2">
            ...
            <div className="text-xs text-muted-foreground capitalize truncate">{user?.role} · {tenant?.name}</div>
          </div>
          {tenant?.slug && (
            <Link href={`/t/${tenant.slug}`} target="_blank">
              <Button variant="outline" size="sm" className="w-full mb-2"><Store className="h-3.5 w-3.5 mr-2" />View storefront</Button>
            </Link>
          )}
