...
import { Plus, Edit, Trash2, FolderTree, Package, X, Settings2, Sparkles } from 'lucide-react'
...
const emptyCategory = { name:'', icon:'🍽️', customVariants:[], customAddons:[], customFlags:{} }
...
export default function AdminCategories() {
  ...
  const openNew = () => { setEditing(null); setF({...emptyCategory}); setOpen(true) }
  const openEdit = (c) => {
    setEditing(c)
    setF({
      name: c.name,
  ...
  const del = async (c) => {
    ...
    if (count > 0) return toast.error(`"${c.name}" has ${count} product(s). Delete or move them first.`)
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Group your menu and define which options products in each category will offer</p>
        </div>
      ...
      {loading ? (
        ...
        <Card className="p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Categories group products and define shared customization options.</p>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {cats.map((c) => {
            ...
            const hasTemplate = (c.customVariants?.length || 0) + (c.customAddons?.length || 0) + Object.values(c.customFlags||{}).filter(Boolean).length > 0
            return (
              <motion.div key={c.id} layout initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.95}}>
                <Card className="p-5 hover:shadow-md transition group">
                  ...
                  <div className="mt-4 flex items-center justify-between gap-2">
                    <Link href={`/admin/products?category=${c.id}`} className="flex-1">
                      <Button size="sm" variant="outline" className="w-full h-8"><Package className="h-3 w-3 mr-1.5"/>Products</Button>
                    </Link>
                    <Link href={`/admin/products?new=1&category=${c.id}`} className="flex-1">
                      <Button size="sm" className="w-full h-8"><Plus className="h-3 w-3 mr-1.5"/>Add product</Button>
                    </Link>
                  </div>
                </Card>
              </motion.div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-[1fr,auto] gap-3">
              <div>
                <Label>Name *</Label>
                <Input placeholder="e.g. Cakes, Bouquets, Meeting Rooms" value={f.name} onChange={e=>setF({...f,name:e.target.value})} autoFocus/>
              </div>
              <div>
                <Label>Icon</Label>
                <Input value={f.icon} onChange={e=>setF({...f,icon:e.target.value})} className="w-20 text-center text-lg" maxLength={4}/>
              </div>
            </div>
            ...
            <div className="border rounded-xl p-4 bg-gradient-to-br from-amber-50/50 to-rose-50/30">
              <div className="flex items-center gap-2 mb-1">
                ...
                <div className="font-semibold text-sm">Product customization template</div>
              </div>
              <p className="text-xs text-muted-foreground mb-4">These options will be pre-filled when you add a new product to this category. You can still tweak per product.</p>

              {/* Boolean flags */}
              <div className="space-y-2 mb-5">
                {FLAG_DEFS.map(fd => (
                  <label key={fd.key} className="flex items-start gap-3 p-2 rounded-lg hover:bg-white cursor-pointer">
                    <Switch checked={!!f.customFlags?.[fd.key]} onCheckedChange={v=>setF({...f, customFlags:{...(f.customFlags||{}), [fd.key]: v}})}/>
                    <div>
                      <div className="text-sm font-medium">{fd.label}</div>
                      <div className="text-xs text-muted-foreground">{fd.hint}</div>
                    </div>
                  </label>
                ))}
              </div>
              ...
              <div className="mb-5">
                ...
                <div className="space-y-3">
                  {(f.customVariants||[]).map((v, vi) => (
                    <Card key={vi} className="p-3 bg-white">
                      <div className="flex gap-2 items-center mb-2">
                        <Input placeholder="Variant name (e.g. Size, Duration)" value={v.name} onChange={e=>updateVariant(vi, {name:e.target.value})} className="flex-1 h-8 text-sm"/>
                  ...
                  {(f.customVariants||[]).length === 0 && <p className="text-xs text-muted-foreground italic">No variants — products in this category will not offer size/style choices.</p>}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  ...
                  {(f.customAddons||[]).length === 0 && <p className="text-xs text-muted-foreground italic">No add-ons — products in this category won't have extras.</p>}
                </div>
              </div>
            </div>

            <Button onClick={save} className="w-full h-11">{editing ? 'Save changes' : 'Add category'}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
