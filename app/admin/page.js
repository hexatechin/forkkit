...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch('/api/admin/analytics', { headers: { Authorization: `Bearer ${t}` }}).then(r=>r.json()),
      fetch('/api/admin/me', { headers: { Authorization: `Bearer ${t}` }}).then(r=>r.json()),
    ]).then(([p, o, a, m]) => {
      setStats({ products: (p.products||[]).length, orders: (o.orders||[]).length, revenue: (o.orders||[]).reduce((s,x)=>s+x.total, 0), categories: (p.categories||[]).length })
    ...
    <div>
      {showWelcome && (
        <Card className="p-5 mb-6 border-orange-200 bg-gradient-to-r from-orange-50 to-rose-50 relative">
          <button onClick={()=>setShowWelcome(false)} className="absolute top-3 right-3 text-neutral-400 hover:text-neutral-700"><X className="h-4 w-4"/></button>
      ...
      <p className="text-sm text-muted-foreground mb-6">{tenant?.name && `Welcome back, ${tenant.name}`}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-3 mb-6">
          ...
          {tenant && (
            <div className="mt-4 space-y-3">
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={copyLink} variant="outline" className="flex-1"><Copy className="h-3.5 w-3.5 mr-1"/>Copy link</Button>
                <Button size="sm" onClick={shareWA} className="flex-1 bg-green-500 hover:bg-green-600"><MessageCircle className="h-3.5 w-3.5 mr-1"/>WhatsApp</Button>
              </div>
        ...
        <Card className="p-5 lg:col-span-2">
          ...
          {analytics.days.length === 0 || stats.orders === 0 ? (
            <div className="h-56 flex items-center justify-center text-sm text-muted-foreground">No orders yet. Share your storefront link to start receiving orders.</div>
          ) : (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          ...
          {analytics.top.length === 0 ? <p className="text-sm text-muted-foreground">No sales yet.</p> : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={analytics.top} layout="vertical" margin={{left: 20}}>
