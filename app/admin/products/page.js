...
function ProductsInner() {
  ...
  const updateVariantOption = (vi, oi, patch) => {
    const nv = f.variants.map((v, i) => i === vi ? { ...v, options: v.options.map((o, j) => j === oi ? { ...o, ...patch } : o) } : v)
    setF({ ...f, variants: nv })
  }
                ...
                return (
                  <section key={c.id}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Button size="sm" variant="outline" onClick={() => openNewInCat(c.id)}><Plus className="h-3.5 w-3.5 mr-1" />Add</Button>
                    </div>
          ...
          <div className="space-y-4">
              ...
              {currentCat && (
                anyExtra ? (
                  <div className="mt-2 text-xs text-orange-600 bg-orange-50 rounded px-2 py-1 inline-flex items-center gap-1">
                    <Sparkles className="h-3 w-3" />Fields below come from "{currentCat.name}" template
                  </div>
                ) : (
                  <div className="mt-2 text-xs text-muted-foreground italic">
                    "{currentCat.name}" has no custom options — only standard fields are shown.
                  </div>
                )
              )}
            </div>
            ...
            {schema.showVariants && (
              <div className="border rounded-lg p-3">
                <Label className="text-sm">Variants (inherited from category)</Label>
                ...
                <p className="text-xs text-muted-foreground mt-2">Option labels are defined in the category. You can tune the per-product price delta above.</p>
              </div>
            )}

            {schema.showAddons && (
              <div className="border rounded-lg p-3">
                <Label className="text-sm">Add-ons (inherited from category)</Label>
                <div className="space-y-1.5 mt-2">
                  {(f.addons || []).map((a, i) => (
                    <div key={i} className="flex gap-2 items-center">
                      <div className="flex-1 text-sm">{a.name}</div>
                      <span className="text-xs text-muted-foreground">₹</span>
                      <Input type="number" value={a.price} onChange={e => updateAddon(i, { price: e.target.value })} className="w-24 h-8 text-sm" />
                    </div>
                  ))}
                </div>
                <p className="text-xs text-muted-foreground mt-2">Add-on names come from the category. You can override the price above.</p>
              </div>
            )}
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }}>
      <Card className="p-3 flex items-center gap-3 hover:shadow-sm transition">
        <img src={p.images?.[0]} onError={(e) => e.currentTarget.style.opacity = '0.3'} className="h-14 w-14 rounded-lg object-cover bg-neutral-100 shrink-0" />
