    ...
    <div className="min-h-screen bg-neutral-50">
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            ...
            {p.images?.length>1 && (
              <div className="mt-3 flex gap-2">
                {p.images.map((im,i) => (
                  <button key={i} onClick={()=>setImgIdx(i)} className={`h-16 w-16 rounded-lg overflow-hidden border-2 ${imgIdx===i?'border-black':'border-transparent'}`}>
          ...
          <div>
            <div className="flex gap-2 mb-2">{p.badges?.map(b => <Badge key={b} style={{background:t.primaryColor,color:'white'}}>{b}</Badge>)}</div>
            ...
            <div className="mt-8 flex items-center gap-4">
              <div className="flex items-center gap-2 border-2 rounded-full px-2 py-1">
                <button onClick={()=>setQty(Math.max(1,qty-1))} className="h-8 w-8 rounded-full hover:bg-neutral-100"><Minus className="h-4 w-4 mx-auto"/></button>
